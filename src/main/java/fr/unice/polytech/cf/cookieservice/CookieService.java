...
public class CookieService {
  ...
  public void submitCookie(Cookie cookie, Account account) {
    if(!(cookie.getStatus().equals(ECookieStatus.DEFAULT))) throw new CookieNotSubmittedException("This cookie can't be submitted to the brand manager");
    if(!(account.getType().equals(EAccountType.CHEF))) throw new UnhautorizedRoleException("Your account can't validate a cookie");
    cookie.setStatus(ECookieStatus.SUBMITTED);
  }
  public void validateCookie(Cookie cookie, Account account) {
    if(!(cookie.getStatus().equals(ECookieStatus.SUBMITTED))) throw new CookieNotSubmittedException("This cookie isn't submitted to the brand manager");
    if(!(account.getType().equals(EAccountType.BRAND_MANAGER))) throw new UnhautorizedRoleException("Your account can't validate a cookie");
    cookie.setStatus(ECookieStatus.VALIDATED);
  }

  public void activateCookie(Cookie cookie, Account account) {
    if(!(cookie.getStatus().equals(ECookieStatus.VALIDATED))) throw new CookieNotValidatedException("This cookie isn't validate by the brand manager");
    if(!(account.getType().equals(EAccountType.CHEF))) throw new UnhautorizedRoleException("Your account can't activate a cookie");
